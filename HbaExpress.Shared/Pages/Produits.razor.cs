...
public partial class Produits
{
    ...
    // Brouillons d'images : original + version détourée (fond blanc), avec le choix
    // du vendeur avant validation. La 1re de la liste devient l'image principale.
    private readonly List<ImageDraft> _drafts = new();

    /// <summary>Un brouillon d'image en cours d'ajout : original, version traitée, statut, choix.</summary>
    private sealed class ImageDraft
    {
        public required ProductImageUpload Original { get; init; }
        public ProductImageUpload? Processed { get; set; }
        public bool IsProcessing { get; set; }
        public string? Error { get; set; }
        public bool UseProcessed { get; set; } = true;

        /// <summary>Version retenue (détourée si dispo et choisie, sinon originale).</summary>
        public ProductImageUpload Selected => UseProcessed && Processed is not null ? Processed : Original;
    }

    private bool _anyProcessing => _drafts.Any(d => d.IsProcessing);
    ...
    private async Task OnFilesSelected(InputFileChangeEventArgs e)
    {
        foreach (var file in e.GetMultipleFiles(maximumFileCount: 10))
        {
            ...
            var original = new ProductImageUpload
            {
                FileName = file.Name,
                ContentType = string.IsNullOrEmpty(file.ContentType) ? "image/jpeg" : file.ContentType,
                Content = ms.ToArray()
            };
            var draft = new ImageDraft { Original = original };
            _drafts.Add(draft);
            StateHasChanged();
            await ProcessDraftAsync(draft);
        }
    }

    /// <summary>Envoie l'image à Cloudinary (via BFF) pour détourage + fond blanc.</summary>
    private async Task ProcessDraftAsync(ImageDraft draft)
    {
        ...
        try
        {
            draft.Processed = await Api.ProcessImageAsync(draft.Original);
            draft.UseProcessed = true; // par défaut, on propose la version détourée
        }
        catch (Exception ex)
        {
            // En cas d'échec, on retombe sur l'originale (le vendeur peut réessayer).
            draft.Processed = null;
            draft.UseProcessed = false;
    ...
    private Task RetryProcess(ImageDraft draft) => ProcessDraftAsync(draft);

    private void UseVersion(ImageDraft draft, bool useProcessed)
    {
        if (useProcessed && draft.Processed is null) return;
        draft.UseProcessed = useProcessed;
    }
    ...
    private void SetPrimary(int index)
    {
        ...
        var d = _drafts[index];
        _drafts.RemoveAt(index);
        _drafts.Insert(0, d); // la 1re image est l'image principale
    }
    ...
    private async Task CreateProduct()
    {
        ...
        if (_anyProcessing)
        {
            _formError = "Traitement des images en cours, veuillez patienter…";
            return;
        }
        ...
        // Images retenues (version détourée choisie, sinon originale), 1re = principale.
        var images = _drafts.Select(d => d.Selected).ToList();

        try
        {
            await Api.CreateProductAsync(request, images);
        }
        catch (Exception ex)
        {
        ...
        await Toast.Success("Produit publié dans votre catalogue 🎉");
